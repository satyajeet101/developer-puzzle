...
import { STOCKS_CONST } from '../../../../../../apps/stocks/src/app/constants/stocks.constant'
...
export class StocksComponent implements OnInit, OnDestroy {
  ...
  public stocksConst = STOCKS_CONST;
  ...
  public ngOnInit(): void {
    this.stockPickerFormSubscription = this.stockPickerForm.get('periodFrom').valueChanges
      .subscribe((periodFromValue) => {
        if (periodFromValue) {
          if ( this.stockPickerForm.controls['periodTo'].value &&
  ...
  public fetchQuote(): void {
    if (this.stockPickerForm.valid) {
      ...
      this.priceQuery.fetchQuote(symbol, { from: periodFrom, to: periodTo });
    }
  }
